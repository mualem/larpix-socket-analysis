...
v2a_mansocket_limits = True # limits used for v2a manual sockets for modules 0 and 1
if v2a_mansocket_limits:
	...
	# Note from 20220913 LMM
	# Need to generate new limits for robot tester.  A number of channels have moved enough to be needed.
	# Biggest examples are 5,10,20,and 21, which basically fail on every chip. 
	# Makes me wonder what was used for the limits on the socket_baselines, 
	# since I think most devices passed on the robot tester.

	limitsdf=pd.DataFrame(columns=['Mean','maxMean','minMean','Std','maxStd','minStd','ChanName','Chan'])
	# adapt values above to the max/min values used for v2b selection 
	# hopefully these will be the same as were used for the v2a
for chan in range(NumASICchannels):
	...
	# first calculate the max and min for std and mean then append to limitsdf
	if MeanMeanbyChan[chan]!=-999:
		theMean=MeanMeanbyChan[chan]
		theErrMean=errMean
		theStd=MeanStdbyChan[chan]
		theErrStd=errStd
		...
		# Try fixed limits for robot socket testing 20220914 
		theMean=20.0
		MaxMean=50.0
		MinMean=3.0
		theStd=2.0
		...
		limitsdf=limitsdf.append({'Mean':theMean,'maxMean':MaxMean,'minMean':MinMean,
			'Std':theStd,'maxStd':MaxStd,'minStd':MinStd,
...
v2b_robotsocket_limits=False # v2b limits used for robot socket testing (fixed limits used)
if v2b_robotsocket_limits:
...
# Get number of valid channels to look for
ExpectedChannels = limitsdf[limitsdf['Mean']!=-999.]['Mean'].count()
print('Got ',ExpectedChannels,' expected channels')
...
for chan in range(NumASICchannels): #[0,1,2]:
	...
	print(chanlimits['Mean'].iat[0])
	if chanlimits['Mean'].iat[0]>0.:  # MedMeanbyChan[chan]>0.:
...
#t2=t2[t2['Mean']==64]
t2=t2[t2['Mean']==ExpectedChannels]
#print (t2.shape)
#print(t2)
...
#ttt=ttt[ttt['Mean']==64]
ttt=ttt[ttt['Mean']==ExpectedChannels]
#print('ttt\n',ttt)
...
#print('Number of Bad Tests done: ',t4[t4['Mean']!=49].shape[0])
print('Number of Bad Tests done: ',t4[t4['Mean']!=ExpectedChannels].shape[0])
